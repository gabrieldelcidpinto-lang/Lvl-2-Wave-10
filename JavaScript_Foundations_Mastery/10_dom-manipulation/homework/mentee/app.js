...
li.dataset.id = task.id; // classList to add class // dataset.id to add 
...
function renderBoard(taskList) {
  ...
  taskList.forEach(function(task){
    ...
    if (task.status === "todo"){
      ...
      inProgress.appendChild(createCard);
    } else if (task.status === "done") {
      done.appendChild(createCard);
    }
  })
}
...
function highlightHighPriority() {
  ...
  highPriority.forEach(function(element){
